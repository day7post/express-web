...
function mockFetch(url, opts){
  const u = String(url);
  let body = {};
  if(u.includes("/treehole/posts") && (!opts || opts.method === "GET" || !opts.method)){
    const kind = (u.match(/kind=([^&]+)/)||[])[1];
    if(u.match(/\/treehole\/posts\/[^?]/)){ body = { post: TREE_ITEMS[0], replies: TREE_ITEMS[0].replies }; }
    else if(kind==="treehole"){ body = { items: TREE_ITEMS, total: TREE_ITEMS.length, page:1, limit:50, has_more:false }; }
  ...
  } else if(u.includes("/reactions") && opts && opts.method==="POST"){
    body = { liked:true, like_count:5 };
  } else if(u.includes("/treehole/posts") && opts && opts.method==="POST"){
  ...
  } else if(u.includes("/treehole/posts") && opts && opts.method==="PUT"){
    body = {};
  }
